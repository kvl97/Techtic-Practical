    ...
    <div class="row">
      <div class="col-lg-12">
        ...
        <div class="kt-portlet">
          ...
          <!--begin::Form-->
        <form class="kt-form kt-form--label-right" id="frmEdit" action="{{ ADMIN_URL }}admin-users/edit/{{ $record->id }}">
            <div class="kt-portlet__body">
              ...
              <div class="form-group row">
                ...
                <div class="col-lg-4 col-md-6 col-sm-12">
                  <div class="input-group">
                    <input type="text" class="form-control required" name="v_phone" placeholder="Cell Number" value="{{ $record->v_phone }}">                    
                  </div>                  
                </div>
              </div>
              ...
              <!-- <div class="form-group row">
                <label class="col-form-label col-md-2 col-lg-2 col-sm-12">Group <span class="required">*</span></label>
                <div class="col-lg-4 col-md-6 col-sm-12 form-group-sub">
                  <select class="form-control required" name="i_group_id" placeholder="Group">
                    <option value="">Select</option>
                    @foreach ($adminGroups as $index => $val)
                    <option value="{{ $index }}" {{ $record->i_group_id == $index ? 'selected=""' : '' }}>{{ $val }}</option>    
                    @endforeach
                  </select>                  
                </div>
              </div>  -->
                ...
                <div class="col-lg-4 col-md-6 col-sm-12 form-group-sub">
                  <select class="form-control required" name="i_role_id" placeholder="Role">
                    ...
                    @foreach ($adminRoles as $index => $val)
                      <option value="{{ $index }}" {{ $record->i_role_id == $index ? 'selected=""' : '' }}>{{ $val }}</option>    
                    @endforeach                    
                  </select>                  
                </div>
              </div> 
              @else
              <input type="hidden" class="form-control required i_role_id" name="i_role_id" value="{{ $record->i_role_id }}"> 
              @endif
              ...
              <div class="form-group row">
                  ...
                  <div class="col-lg-4 col-md-6 col-sm-12">
                      <div class="fileinput" data-provides="fileinput">
                          ...
                          if (File::exists(ADMIN_USER_PROFILE_IMG_PATH . $record['v_profile_image']) && $record['v_profile_image'] != '') {
                              echo SITE_URL . ADMIN_USER_PROFILE_IMG_PATH . $record['v_profile_image'];
                          } else {
                          ...
                          <div class="cropper_area_elements" style="display:none;">
                              <img width="{{ File::exists(ADMIN_USER_PROFILE_IMG_PATH . $record['v_profile_image']) && $record['v_profile_image'] != '' ? '300px' : '300px' }}" src="<?php
                              if (File::exists(ADMIN_USER_PROFILE_IMG_PATH . $record['v_profile_image']) && $record['v_profile_image'] != '') {
                                  echo SITE_URL . ADMIN_USER_PROFILE_IMG_PATH . $record['v_profile_image'];
                              } else {
                                  echo ASSET_URL . 'images/default-image.png';
                          ...
                          <img  width="300px" src="{{ ASSET_URL.'images/default-image.png'}}" class="img-responsive default_img_size" id="defailt_profile_pic" style="display: none;"/>
                      </div>
                      <div class="clearfix"></div>
                      <div style="margin-top:20px">
                          <button class="btn btn-default profile_preview_elements" type="button" id="edit_image" style="display: {{  File::exists(ADMIN_USER_PROFILE_IMG_PATH.$record['v_profile_image']) && $record['v_profile_image'] !='' ? 'inline-block' : 'none' }};">Edit</button>	
                          <button class="btn btn-default" style="display: {{  File::exists(ADMIN_USER_PROFILE_IMG_PATH.$record['v_profile_image']) && $record['v_profile_image'] !='' ? 'none' : 'inline-block' }};" type="button" id="file_trriger"><?php echo File::exists(ADMIN_USER_PROFILE_IMG_PATH . $record['v_profile_image']) && $record['v_profile_image'] != '' ? 'Change' : 'Select Image'; ?></button>
                          <span class="cropper_area_elements" style="display:none;">
                              <button class="btn btn-default" type="button" id="remove_image" style="display: {{  File::exists(ADMIN_USER_PROFILE_IMG_PATH.$record['v_profile_image']) && $record['v_profile_image'] !='' ? 'inline-block' : 'none' }};">Remove</button>
                              
                              <button class="btn btn-default" type="button" id="cancel_image_crop" style="display: {{  File::exists(ADMIN_USER_PROFILE_IMG_PATH.$record['v_profile_image']) && $record['v_profile_image'] !='' ? 'inline-block' : 'none' }};">Cancel</button>
                          </span>
