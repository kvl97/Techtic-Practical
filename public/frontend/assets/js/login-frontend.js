            ...
            if(!form_valid('#forgot_form')) {
                ...
                $.post(action,data,function(response) {
                    ...
                    if(response.status == 'TRUE') {
                        ...
                        $("#error_msg_forgot_passwword").html('<div class="alert alert-danger invalid-error-message alert-dismissible" role="alert"><span type="button" class="close" data-dismiss="alert" aria-label="Close"><span aria-hidden="true">&times;</span></span>'+response.message+'</div>');
                        $("html, body").animate({
                            scrollTop: 0
                        }, 1000);
                    }
        ...
        $(document).on('submit','#MyProfileFrm',function(event){
            ...
            if(!form_valid('#MyProfileFrm')) {
                return false;
                    ...
                    var form = $(this).closest('form');
                    $(this).find('#my_profile_page').attr('disabled',true);
                    $.post(action,data,function(response) {
                        if(response.status == 'TRUE') {
                            window.location.href = response.redirect_url;
                        } else {
                            $(this).find('#my_profile_page').removeAttr('disabled');
                            response = $.parseJSON(response);
                            $(response).each(function (i, val) {
                                $.each(val, function (key, v) {
                                    $('#' + key).closest('.form-group').addClass('is-invalid');
